...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ------------------------------------------------------------------------
  // 7. LIVE PHOTO REPLACEMENT & PERSISTENCE
  // ------------------------------------------------------------------------
  ...
  // Load stored custom photos from LocalStorage if present
  function loadSavedPhotos() {
    const saved = localStorage.getItem('gifa_custom_photos');
    if (saved) {
      try {
        const photos = JSON.parse(saved);
        Object.keys(photos).forEach(slotId => {
          const targetImg = document.querySelector(`[data-photo-slot="${slotId}"]`);
          if (targetImg) targetImg.src = photos[slotId];
        });
      } catch (err) {
        console.error('Failed loading saved photos', err);
      }
    }
  }
  loadSavedPhotos();
  ...
  // Also enable direct click on polaroid frames to trigger photo upload for that slot!
  document.querySelectorAll('[data-photo-slot]').forEach(imgEl => {
    const slotId = imgEl.getAttribute('data-photo-slot');
    const parentPolaroid = imgEl.closest('.polaroid-item, .polaroid-hero-frame');
    if (parentPolaroid) {
      parentPolaroid.addEventListener('click', (e) => {
        // Only if not clicking a button inside
        if (e.target.tagName !== 'BUTTON' && uploadModal && targetSlotSelect) {
          targetSlotSelect.value = slotId;
  ...
  function handleFileSelect(file) {
    ...
    reader.onload = (e) => {
      ...
      const slotId = targetSlotSelect ? targetSlotSelect.value : 'hero';
      const targetImg = document.querySelector(`[data-photo-slot="${slotId}"]`);
      if (targetImg) {
        ...
        let photos = {};
        try {
          photos = JSON.parse(localStorage.getItem('gifa_custom_photos') || '{}');
        } catch (err) {}
        photos[slotId] = dataUrl;
        localStorage.setItem('gifa_custom_photos', JSON.stringify(photos));

        audio.playChime();
        if (uploadModal) uploadModal.classList.remove('active');

        // Confetti burst on photo update
